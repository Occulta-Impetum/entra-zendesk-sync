...
def _desired_from_mapping(user_id: str, user: dict[str, Any], mapping: dict[str, Any]) -> dict[str, Any]:
    zendesk_org = mapping.get("zendesk_organization") or {}
    entra_group = mapping.get("entra_group") or {}
    manager = user.get("manager") if isinstance(user.get("manager"), dict) else None
    return {
        "entra_id": user_id,
        ...
        "job_title": str(user.get("jobTitle") or "").strip(),
        "manager_entra_id": str((manager or {}).get("id") or "").strip(),
        "manager_name": str((manager or {}).get("displayName") or "").strip(),
        "manager_email": _norm_email((manager or {}).get("mail") or (manager or {}).get("userPrincipalName")),
    ...
    for entra_id, desired in desired_users.items():
        ...
        if len(external_matches) > 1:
            if decision == "skip":
                ...
                row = _row(desired, "CONFLICT", "Multiple Zendesk users share this Entra external_id.")
        ...
        if len(external_matches) == 1:
            ...
            if len(email_matches) > 1:
                ...
                selected = next((item for item in email_matches if str(item.get("id")) == str(selected_id)), None)
                if decision == "use_zendesk_user" and selected is not None:
                    ...
                    row = _row(desired, "CONFLICT", "Multiple Zendesk users match the Entra email address.")
        ...
        if zendesk_user is None:
            if decision == "skip":
                ...
                plan.append(_row(desired, "NO CHANGE", "Entra account is disabled and no Zendesk user exists."))
            elif allow_email_bootstrap:
        ...
        if protect_zendesk_staff_roles and role in STAFF_ROLES:
            plan.append(_row(desired, "PROTECTED", f"Matched Zendesk {role}; staff roles are protected from sync changes.", zendesk_user, matched_by))
            continue
        ...
        if matched_by == "email" and existing_external and existing_external != external_id:
            ...
            if decision == "replace_external_id" and allowed_match:
                ...
                plan.append(_row(desired, "SKIP", "Administrator chose not to adopt the email-matched Zendesk user.", zendesk_user, matched_by))
                continue
            else:
                row = _row(desired, "CONFLICT", "Email matched a Zendesk user that already has a different external_id.", zendesk_user, matched_by)
        ...
        if not desired["enabled"] and suspend_when_entra_disabled:
            ...
            if str(current_org_id or "") != str(desired["zendesk_org_id"]):
                ...
                reasons.append(f"organization {current_org_id or '-'} -> {desired['zendesk_org_id']}")

        if not actions:
            actions = ["NO CHANGE"]
            reasons = ["Zendesk state already matches desired Entra state."]
        plan.append(_row(desired, " + ".join(actions), "; ".join(reasons), zendesk_user, matched_by))
...
def add_user_field_actions(
    ...
    *,
    ...
    title_key = field_keys["job_title"]

    for entra_id, desired in desired_users.items():
        row = plan_by_entra.get(entra_id)
        if not row or str(row.get("action") or "") in {"CONFLICT", "SKIP", "PROTECTED"}:
            continue
        actions = [piece.strip() for piece in str(row.get("action") or "").split("+") if piece.strip()]
        if actions == ["NO CHANGE"]:
            actions = []
            row["reason"] = ""
        reasons = [part.strip() for part in str(row.get("reason") or "").split(";") if part.strip()]
        zendesk_user = by_zendesk_id.get(int(row["zendesk_id"])) if row.get("zendesk_id") is not None else None
        existing_fields = zendesk_user.get("user_fields") if isinstance(zendesk_user, dict) else {}
        if not isinstance(existing_fields, dict):
            existing_fields = {}

        desired_employee = str(desired.get("employee_id") or "").strip()
        current_employee = str(existing_fields.get(employee_key) or "").strip()
        if desired_employee != current_employee:
            actions.append("UPDATE EMPLOYEE ID")
            reasons.append(f"employee ID {current_employee or '-'} -> {desired_employee or '-'}")

        desired_title = str(desired.get("job_title") or "").strip()
        current_title = str(existing_fields.get(title_key) or "").strip()
        if desired_title != current_title:
...
def _row(
    ...
    row = {
        ...
        "reason": reason,
        "employee_id": desired.get("employee_id", ""),
        "job_title": desired.get("job_title", ""),
        "manager_entra_id": desired.get("manager_entra_id", ""),
        "manager_name": desired.get("manager_name", ""),
        "manager_email": desired.get("manager_email", ""),
    }
    if zendesk_user:
