...
from lib.cache import CacheError, load_entra_users_cache, save_entra_users_cache, save_zendesk_users_cache
...
def _validate_apply_plan(plan: list[dict[str, Any]], unresolved_conflicts: list[dict[str, Any]], unresolved_reviews: list[dict[str, Any]]) -> None:
    if unresolved_conflicts:
        raise BootstrapApplyError(f"Refusing bootstrap apply: {len(unresolved_conflicts)} unresolved conflict(s) remain.")
    if unresolved_reviews:
        raise BootstrapApplyError(f"Refusing bootstrap apply: {len(unresolved_reviews)} initial match review(s) remain.")
    if any("UPDATE EMAIL" in _action_parts(row) for row in plan):
        raise BootstrapApplyError("Refusing bootstrap apply because the live plan contains UPDATE EMAIL actions. Zendesk primary-email changes require the User Identities API and are intentionally not enabled yet.")
    invalid_creates = [row for row in plan if "CREATE" in _action_parts(row) and (not str(row.get("email") or "").strip() or not str(row.get("name") or "").strip())]
    if invalid_creates:
        raise BootstrapApplyError(f"Refusing bootstrap apply: {len(invalid_creates)} CREATE row(s) are missing name or email.")


def _confirm_apply(counts: Counter[str], total_rows: int) -> bool:
    _print_summary(counts, total_rows)
    print("\nThis is the one-time bootstrap migration. It will create/modify Zendesk end users and, if needed, create the Employee ID user field.\nExisting email-matched users will keep their Zendesk ticket history.\nType APPLY exactly to continue, or press Enter to cancel.")
    try:
...
def _write_row_first_pass(row: dict[str, Any], *, access_token: str, subdomain: str, field_keys: dict[str, str]) -> str:
    actions = _action_parts(row)
    ...
    if "CREATE" in actions:
        create_user(access_token, subdomain, name=str(row.get("name") or "").strip(), email=str(row.get("email") or "").strip(), external_id=external_id, organization_id=int(row["zendesk_org_id"]), user_fields=user_fields)
        return "CREATED"
    zendesk_id = row.get("zendesk_id")
    if zendesk_id is None:
        raise BootstrapApplyError(f"Plan row for {row.get('name') or entra_id} requires an update but has no Zendesk user id.")
    fields: dict[str, Any] = {}
...
def _apply_manager_second_pass(plan: list[dict[str, Any]], *, access_token: str, subdomain: str, manager_key: str) -> tuple[int, int]:
    manager_rows = [row for row in plan if _action_parts(row) & {"UPDATE MANAGER", "CLEAR MANAGER"} and not (_action_parts(row) & {"SKIP", "PROTECTED", "CONFLICT"})]
    if not manager_rows:
    ...
    for index, row in enumerate(manager_rows, start=1):
        subject = by_external.get(f"{EXTERNAL_ID_PREFIX}{row['entra_id']}".lower())
        if not subject:
        ...
        if "UPDATE MANAGER" in actions:
            ...
            if manager is None:
                ...
                if len(matches) == 1:
                    manager = matches[0]
            if manager is None:
                print(f"      [{index}/{len(manager_rows)}] WARNING: manager target not found for {row.get('name')} -> {row.get('manager_name') or row.get('manager_email') or manager_entra_id}")
                unresolved += 1
                continue
            manager_value = int(manager["id"])
        print(f"      [{index}/{len(manager_rows)}] {row.get('name')}: manager -> {manager_value if manager_value is not None else '-'}...", flush=True)
        update_user(access_token, subdomain, int(subject["id"]), fields={"user_fields": {manager_key: manager_value}})
        updated += 1
...
def _expected_link_ids(plan: list[dict[str, Any]]) -> set[str]:
    ...
    for row in plan:
        entra_id = str(row.get("entra_id") or "").strip()
        if entra_id and (_action_parts(row) & {"CREATE", "ADOPT", "RELINK"} or row.get("matched_by") == "external_id"):
            expected.add(entra_id)
    return expected


def _verify_after_apply(*, expected_link_ids: set[str], access_token: str, subdomain: str) -> tuple[int, int]:
    print("\nPost-apply verification: refreshing Zendesk users...", flush=True)
    users = get_users(access_token, subdomain)
    save_zendesk_users_cache(users, subdomain=subdomain)
    external_ids = {str(user.get("external_id") or "").strip().lower() for user in users if user.get("external_id")}
    missing = [entra_id for entra_id in sorted(expected_link_ids) if f"{EXTERNAL_ID_PREFIX}{entra_id}".lower() not in external_ids]
    if missing:
...
def _seed_operational_cache(plan: list[dict[str, Any]]) -> None:
    """Seed the incremental Entra baseline from the successfully applied bootstrap plan."""
    current: dict[str, dict[str, Any]] = {}
    for row in plan:
        entra_id = str(row.get("entra_id") or "").strip()
        if not entra_id or not row.get("group_id"):
            continue
        current[entra_id] = {
            "entra_id": entra_id,
            ...
            "manager_email": str(row.get("manager_email") or "").strip().lower(),
            "zendesk_org_id": int(row["zendesk_org_id"]),
            "zendesk_org_name": str(row.get("zendesk_org_name") or ""),
        }
    path = save_entra_users_cache(current, previous=load_entra_users_cache())
    print(f"      Incremental Entra baseline saved: {path} ({len(current)} user(s)).")
...
def run_bootstrap_apply() -> int:
    ...
    print("=======================================")
    try:
        config, plan, _in_scope_ids, zendesk_config, zendesk_token, unresolved_conflicts, unresolved_reviews, field_keys = _build_live_bootstrap_plan()
        ...
        field_keys = ensure_user_fields(config=config, access_token=zendesk_token, subdomain=zendesk_config["subdomain"], allow_create=True)
        actionable = [row for row in plan if _action_parts(row) - {"NO CHANGE", "SKIP", "PROTECTED", "UPDATE MANAGER", "CLEAR MANAGER"}]
        print(f"\nFirst pass: applying identity, organization, employee ID, and job title for {len(actionable)} row(s)...", flush=True)
        created = updated = skipped = 0
        for index, row in enumerate(plan, start=1):
            first_pass_actions = _action_parts(row) - {"UPDATE MANAGER", "CLEAR MANAGER"}
            ...
            result = _write_row_first_pass(row, access_token=zendesk_token, subdomain=zendesk_config["subdomain"], field_keys=field_keys)
            if result == "CREATED": created += 1
            elif result == "UPDATED": updated += 1
            else: skipped += 1
        manager_updated, manager_unresolved = _apply_manager_second_pass(plan, access_token=zendesk_token, subdomain=zendesk_config["subdomain"], manager_key=field_keys["manager"])
        ...
        total_expected, missing_links = _verify_after_apply(expected_link_ids=expected_link_ids, access_token=zendesk_token, subdomain=zendesk_config["subdomain"])
        if missing_links:
            print(f"\nBOOTSTRAP APPLY FINISHED WITH VERIFICATION WARNINGS. {missing_links} of {total_expected} expected Entra links are missing.")
            return 1
        if manager_unresolved:
            print(f"\nBOOTSTRAP APPLY FINISHED WITH MANAGER WARNINGS. {manager_unresolved} manager relationship(s) could not be resolved; operational baseline was not saved.")
            return 1
        print("\nSeeding incremental operational cache from successful bootstrap state...", flush=True)
        _seed_operational_cache(plan)
        print("\nBOOTSTRAP APPLY COMPLETE.")
        print("All expected bootstrap identities are linked by immutable entra:<object-id> external IDs.")
        print("The incremental Entra baseline is ready for scheduled operational sync.")
        return 0
    except (BootstrapApplyError, BootstrapReviewError, CacheError, ConfigError, ConflictSnapshotError, GraphError, ResolutionError, UserFieldSetupError, ZendeskError, KeyError, ValueError) as exc:
        print(f"\nERROR: {exc}")
        print("Bootstrap apply stopped. Any writes completed before an API failure remain in Zendesk; re-running is safe because the live plan is rebuilt before every attempt.")
        return 1
