...
class IncrementalNoChangeTests(unittest.TestCase):
    ...
    def test_no_change_skips_zendesk_authentication(
        self,
        collect_mock,
        cache_mock,
        zendesk_config_mock,
        ...
        current = {"id-1": {"entra_id": "id-1", "name": "Same"}}
        previous = {"version": 1, "current": current, "history": current}
        collect_mock.return_value = ({"zendesk": {"user_fields": {}}}, current, [])
        cache_mock.return_value = previous

        plan, returned_current, returned_previous = build_incremental_plan()

        self.assertEqual(plan, [])
        self.assertEqual(returned_current, current)
        self.assertEqual(returned_previous, previous)
        zendesk_config_mock.assert_not_called()
...
class IncrementalEmailReusePlanTests(unittest.TestCase):
    ...
    def _previous_with_history(self) -> dict:
        return {
            "version": 1,
            "current": {},
            "history": {
    ...
    def test_reused_email_requires_retired_managed_identity_and_history(
        ...
        collect_mock.return_value = (self._config(), self._current(), [])
        cache_mock.return_value = self._previous_with_history()
    ...
    @patch("lib.operational.load_zendesk_config")
    @patch("lib.operational.collect_current_entra_state")
    def test_unsuspended_old_owner_is_conflict(
        self,
        collect_mock,
        zendesk_cfg_mock,
        token_mock,
        ...
        self.assertIn("suspended", plan[0]["reason"])
