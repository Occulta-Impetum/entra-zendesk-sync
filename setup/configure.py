...
The GUI discovers Entra security groups and Zendesk organizations, lets the
administrator select groups with checkboxes, maps each selected group to a
Zendesk organization, and saves non-secret configuration to config/config.yaml.
"""
...
class ScrollableFrame(ttk.Frame):
    """A vertically scrollable frame suitable for long checkbox/form lists."""
...
class ConfigureApp(tk.Tk):
    ...
    def _show_loading_page(self) -> None:
        ...
        ttk.Label(
            self.container,
            text="Entra -> Zendesk Sync Configuration",
            font=("Segoe UI", 16, "bold"),
        ).pack(anchor="w", pady=(0, 10))
        ttk.Label(
            self.container,
            text="Connecting to Microsoft Entra and Zendesk. This may take a moment.",
        ).pack(anchor="w", pady=(0, 14))
        ttk.Progressbar(self.container, mode="indeterminate").pack(fill="x", pady=(0, 14))
        progress = self.container.winfo_children()[-1]
        progress.start(10)
        ttk.Label(self.container, textvariable=self.status_var).pack(anchor="w")

    def _begin_discovery(self) -> None:
        thread = threading.Thread(target=self._discover_data, daemon=True)
        thread.start()
    ...
    def _discover_data(self) -> None:
        try:
            ...
            self.organizations = get_organizations(
                zendesk_token,
                self.zendesk_config["subdomain"],
            )

            self.selected_group_ids = {
                group_id
                for group_id in self.existing_mappings
    ...
    def _show_group_page(self) -> None:
        ...
        ttk.Label(
            self.container,
            text="1. Select Entra Security Groups",
            font=("Segoe UI", 16, "bold"),
        ).pack(anchor="w")
        ttk.Label(
            self.container,
            text=(
                "Choose the Entra groups that define Zendesk provisioning scope. "
                "Existing selections are checked automatically."
            ),
            wraplength=800,
        ...
        ttk.Button(action_frame, text="Select All Visible", command=self._select_all_visible).pack(
            side="left"
        )
        ttk.Button(action_frame, text="Clear Visible", command=self._clear_visible).pack(
            side="left", padx=(8, 0)
        )
        self.selection_count_var = tk.StringVar()
        ttk.Label(action_frame, textvariable=self.selection_count_var).pack(side="left", padx=16)
        ttk.Button(action_frame, text="Next: Map Organizations >", command=self._go_to_mapping).pack(
            side="right"
        )
    ...
    def _visible_groups(self) -> list[dict]:
        ...
        return [
            ...
            if query in str(group.get("displayName") or "").lower()
    ...
    def _render_group_list(self) -> None:
        ...
        if not visible:
            ttk.Label(self.group_scroll.inner, text="No groups match the current search.").pack(
            ...
            for group in visible:
                ...
                if description:
                    ttk.Label(row, text=description, foreground="#666666", wraplength=720).pack(
                        anchor="w", padx=(24, 0)
                    )
    ...
    def _go_to_mapping(self) -> None:
        selected = {
            group_id for group_id, var in self.group_vars.items() if var.get()
        }
        if not selected:
            messagebox.showwarning(
                "No groups selected",
                "Select at least one Entra security group before continuing.",
            )
            return
    ...
    def _show_mapping_page(self) -> None:
        ...
        ttk.Label(
            self.container,
            text="2. Map Groups to Zendesk Organizations",
            font=("Segoe UI", 16, "bold"),
        ).pack(anchor="w")
        ttk.Label(
            self.container,
            text=(
                "Choose the Zendesk organization for each selected Entra group. "
                "Immutable IDs will be saved; names are kept for readability."
            ),
            wraplength=800,
        ).pack(anchor="w", pady=(4, 12))
        ...
        org_display_to_org = {
            self._org_display(org): org for org in self.organizations
        }
        org_values = list(org_display_to_org.keys())
        self.org_display_to_org = org_display_to_org

        selected_groups = [
            group for group in self.groups if str(group.get("id")) in self.selected_group_ids
        ]
        ...
        for group in selected_groups:
            ...
            if not var.get():
                ...
                existing_org = existing.get("zendesk_organization") or {}
                existing_org_id = str(existing_org.get("id") or "")
    ...
    def _save(self) -> None:
        ...
        if missing:
            ...
            messagebox.showwarning(
                "Mappings incomplete",
                "Every selected Entra group must have a Zendesk organization.\n\n" + preview,
            )
            return
